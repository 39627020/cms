...
public class SessionAuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if(CmsConfig.isDebug){
			System.out.println("-------Session Api Interceptor: "+url);
		}
		...
		if(!customAuths.contains(aname)){
			...
			LoginSession loginSession = LoginSessionUtil.getLoginMapper().selsetBySession(sessionid);
	...
	private Set<String> getAllActions(HttpSession httpSession,int userid){
		...
		//查询每个角色对应的权限
		for(int rid : LoginSessionUtil.getUserService().listUserRoleIds(userid)){
			//数据库中role id 从1开始
